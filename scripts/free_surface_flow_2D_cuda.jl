...
function update_τ!(ηeff,τxy,τxz,vx,re,ny,cfl,dy,dz)
    iy = (blockIdx().x-1)*blockDim().x + threadIdx().x
    iz = (blockIdx().y-1)*blockDim().y + threadIdx().y
    if iy<=size(τxy,1) && iz<=size(τxy,2)   τxy[iy,iz]  = τxy[iy,iz] + (-τxy[iy,iz] + @avz(ηeff)*@d_yi(vx)/dy)/(1.0 + 3.0*cfl*ny/re) end
    if iy<=size(τxz,1) && iz<=size(τxz,2)   τxz[iy,iz]  = τxz[iy,iz] + (-τxz[iy,iz] + @avy(ηeff)*@d_zi(vx)/dz)/(1.0 + 3.0*cfl*ny/re) end
    return
end

function update_v!(vx,τxy,τxz,ηeff,ρg,sinα,vdτ,lz,re,dy,dz)
    iy = (blockIdx().x-1)*blockDim().x + threadIdx().x
    iz = (blockIdx().y-1)*blockDim().y + threadIdx().y
    if iy<=size(vx,1)-2 && iz<=size(vx,2)-2
        vx[iy+1,iz+1] = vx[iy+1,iz+1] + (@d_ya(τxy)/dy + @d_za(τxz)/dz + ρg*sinα)*(vdτ*lz/re)/@av4(ηeff) end
    return
end
...
@views function main()
    ...
    while err >= ϵtol && iter <= maxiter
        ...
        CUDA.@sync @cuda threads=nthreads blocks=nblocks update_τ!(ηeff,τxy,τxz,vx,re,ny,cfl,dy,dz)
        CUDA.@sync @cuda threads=nthreads blocks=nblocks update_v!(vx,τxy,τxz,ηeff,ρg,sinα,vdτ,lz,re,dy,dz)
