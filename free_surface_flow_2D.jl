...
@views function main()
    ...
    while err >= ϵtol && iter <= maxiter
        ...
        bc2!(ηeff)
        τxy                  .+= (.-τxy .+ avy(ηeff).*diff(vx,dims=1)./dy)./(1.0 + 1.5cfl*ny/re)
        τxz                  .+= (.-τxz .+ avz(ηeff).*diff(vx,dims=2)./dz)./(1.0 + 1.5cfl*ny/re)
        ...
        if iter % ncheck == 0
            ...
            display(plot(p1,p2,p3;size=(1600,300),layout=(1,3),bottom_margin=10mm,left_margin=10mm))
